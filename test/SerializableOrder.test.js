...
contract('SerializableOrder', function([_, user]) {
    ...
    describe('deserialize', function() {
        ...
        it('get main token ID', async function() {
            let order_data = await this.SerializableOrder.getOrderTokenIDMainMock.call(ser_hex_0);
            order_data.should.be.bignumber.eq(mainToken);
        });

        it('get main amount', async function() {
            let order_data = await this.SerializableOrder.getOrderAmountMainMock.call(ser_hex_0);
            order_data.should.be.bignumber.eq(mainAmount);
        });

        it('get sub token ID', async function() {
            let order_data = await this.SerializableOrder.getOrderTokenIDSubMock.call(ser_hex_0);
            order_data.should.be.bignumber.eq(subToken);
        });

        it('get sub amount', async function() {
            let order_data = await this.SerializableOrder.getOrderAmountSubMock.call(ser_hex_0);
        ...
        it('get fee price', async function() {
            let order_data = await this.SerializableOrder.getOrderFeePriceMock.call(ser_hex_0);
...
contract('SerializableOrder_v2', function([_, user]) {
    beforeEach(async function() {
        this.SerializableOrder = await SerializableOrderMock.new();
    });
    const userID = 11;
    ...
    describe('serialize', function() {
        it('normal order', async function() {
            let ser_data = await this.SerializableOrder.serializeOrder_v2.call(
                userID,
                tokenIDTarget,
                amountTarget,
                tokenIDTrade,
    ...
    describe('deserialize', function() {
        it('normal hex', async function() {
            let order_data = await this.SerializableOrder.deserializeOrder_v2.call(ser_hex_0);
            order_data[0].should.be.bignumber.eq(userID);
